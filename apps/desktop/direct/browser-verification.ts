...
export interface DirectBrowserContract {
  ...
  readonly probe: DirectProbeSnapshot;
}
...
function directCatalogIdentity(manifest: DirectSessionManifest): string {
 ...
 */
export function bindDirectScenarioCatalog(
  manifests: readonly DirectSessionManifest[],
): DirectSessionManifest["coverage"] {
...
export function bindDirectBrowserContractEvidence(
  initial: DirectBrowserContract,
  final: DirectBrowserContract,
  retainedProbe: DirectProbeSnapshot = final.probe,
): DirectBrowserContract {
...
export async function readDirectBrowserContract(
  ...
  if (envelope.bridgeSchema !== DIRECT_BROWSER_BRIDGE_SCHEMA) {
    throw new Error(
      `Direct browser bridge schema must be ${DIRECT_BROWSER_BRIDGE_SCHEMA}`,
    );
  }
  const manifest = parseDirectSessionManifest(envelope.manifest);
  if (!manifest.ok) {
    throw new Error(`Direct session manifest is invalid: ${manifest.error.message}`);
  }
  const probe = parseDirectProbeSnapshot(envelope.probe);
  if (!probe.ok) {
...
export function isolatedAgentBrowserEnvironment(options: {
  ...
  return {
    ...
    AGENT_BROWSER_IDLE_TIMEOUT_MS: String(options.defaultTimeoutMs + 60_000),
...
export function createAgentBrowser(options: {
  ...
  const createEnvironment = () => {
    ...
    return isolatedAgentBrowserEnvironment({
      configPath: join(import.meta.dir, "agent-browser.verify.json"),
