import { z } from "zod";
import { SCENE_UPLOAD_POLICY } from "@hraness/slopcamera/scene";
...
const TOOL = Object.freeze({ name: "slopcamera-core", profile: "direct-fixture", version: "1" });
...
function screenAsset(): ProjectAssetV1 {
  const path = "artifacts/slopcamera/private/imports/screen-demo.mp4";
...
function createAlignmentAnalyses(): readonly [AudioAlignmentAnalysisV1, AudioAlignmentAnalysisV1] {
  ...
  const common = {
    ...
    createdAt: TIMESTAMP,
    kind: "slopcamera.audio-alignment-analysis" as const,
    reference,
...
function createMusicAnalysis() {
  return MusicAnalysisV1Schema.parse({
    ...
    tool: { name: "slopcamera-music", profile: "local-deterministic", version: "1" },
  });
}
...
function sceneAnalysis(options: {
  ...
  const samples = options.scenes.map((scene, index) => {
    ...
    return {
      ...
      path: `artifacts/slopcamera/projects/${ID.project}/analysis/scenes/${options.pathStem}-${String(index + 1)}.jpg`,
  ...
  return SceneAnalysisV1Schema.parse({
    ...
    scenes: options.scenes.map((scene, index) => ({
      ...
      description: {
        ...
        setting: scene.contentKind === "camera" ? "Desk camera angle" : "Slopcamera terminal workspace",
...
function assetForOverlay(kind: "emoji" | "gif" | "image" | "svg" | "video") {
  const generated = {
    command: ["slopcamera", "overlay", "prepare", kind],
    generator: "slopcamera-overlay-prep",
    generatorVersion: "1",
  ...
  return {
    asset: {
      ...
      mediaType,
      path: `artifacts/slopcamera/projects/${ID.project}/assets/${kind}.${kind === "image" || kind === "emoji" ? "png" : kind === "video" ? "mp4" : kind}`,
...
export function createProjectEvidence(): ProjectEvidence {
  ...
  const cameraA = cameraAsset({
    ...
    path: "artifacts/slopcamera/private/imports/camera-a.mp4",
    sha256: SHA.cameraA,
  ...
  const cameraB = cameraAsset({
    ...
    path: "artifacts/slopcamera/private/imports/camera-b.mp4",
    sha256: SHA.cameraB,
  ...
  const screenScenes = sceneAnalysis({
    ...
    pathStem: "screen",
    scenes: [
      { contentKind: "terminal", endUs: 14_000_000, summary: "The CLI creates a synchronized multi-angle project.", title: "Create project", visibleTextSummary: "slopcamera projects create and project add" },
      { contentKind: "terminal", endUs: SCREEN_DURATION_US, summary: "The timeline applies a global cut, speed range, and overlays.", title: "Edit project", visibleTextSummary: "slopcamera project edit, align analyze, render" },
    ],
    streamId: ID.streamScreenVideo,
  });
  ...
  const analyses = [
    { analysisId: ID.alignA, confidence: 0.98, createdAt: TIMESTAMP, driftPpm: 0, kind: "audio-alignment", path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.alignA}.json`, referencePlacementId: ID.placementScreen, sha256: SHA.alignA, targetPlacementId: ID.placementCameraA },
    { analysisId: ID.alignB, confidence: 0.95, createdAt: TIMESTAMP, driftPpm: -24, kind: "audio-alignment", path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.alignB}.json`, referencePlacementId: ID.placementScreen, sha256: SHA.alignB, targetPlacementId: ID.placementCameraB },
    { analysisId: ID.music, assetId: ID.assetScreen, createdAt: TIMESTAMP, keyRegions: music.keyRegions.length, kind: "music", musicRegions: music.musicRegions.length, path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.music}.json`, sha256: SHA.music, streamId: ID.streamScreenAudio, tempoRegions: music.tempoRegions.length },
    { analysisId: ID.sceneScreen, assetId: ID.assetScreen, createdAt: TIMESTAMP, kind: "scenes", model: screenScenes.model.requestedModel, path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.sceneScreen}.json`, sceneCount: screenScenes.scenes.length, sha256: SHA.sceneScreen, streamIds: [ID.streamScreenVideo] },
    { analysisId: ID.sceneCamera, assetId: ID.assetCameraA, createdAt: TIMESTAMP, kind: "scenes", model: cameraScenes.model.requestedModel, path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.sceneCamera}.json`, sceneCount: cameraScenes.scenes.length, sha256: SHA.sceneCamera, streamIds: [ID.streamCameraAVideo] },
    { analysisId: ID.faceAnalysis, analyzedFrames: faceAnalysis.coverage.analyzedFrames, assetId: ID.assetCameraA, createdAt: TIMESTAMP, kind: "faces", localOnly: true, path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.faceAnalysis}.json`, sha256: SHA.face, streamId: ID.streamCameraAVideo, subjectIntegritySha256: SHA.cameraA, trackCount: faceAnalysis.tracks.length },
    { analysisId: ID.speech, assetId: ID.assetCameraA, createdAt: TIMESTAMP, fillerCount: speech.result.status === "transcribed" ? speech.result.fillers.length : 0, kind: "speech", path: `artifacts/slopcamera/projects/${ID.project}/analysis/${ID.speech}.json`, sha256: SHA.speech, streamId: ID.streamCameraAAudio, wordCount: speech.result.status === "transcribed" ? speech.result.words.length : 0 },
  ];
  const project = VideoProjectV1Schema.parse({
    analyses,
    assets: [screen, cameraA, cameraB],
    createdAt: TIMESTAMP,
    currentEditPlanPath: `artifacts/slopcamera/projects/${ID.project}/edits/plan_agentdemo1.json`,
  ...
  const nextCommands = (cameraMoveId: string) => ({
    remove: `slopcamera project edit ${ID.project} camera remove ${cameraMoveId} --json`,
    show: `slopcamera project edit ${ID.project} camera show --json`,
  });
