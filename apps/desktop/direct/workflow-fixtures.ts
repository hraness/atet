...
const RUN_PATH = `artifacts/slopcamera/private/workflow-runs/${RUN_ID}`;
...
export const DIRECT_WORKFLOW_OPERATION_DISCOVERY = [
  {
    inputSchemaId: "slopcamera.operation.analysis.faces.input/v1",
    kind: "analysis.faces",
    lifecycle: "local-artifact",
    outputSchemaId: "slopcamera.operation.analysis.faces.output/v1",
    policy: {
      cache: "none",
      cancellable: true,
  ...
  {
    inputSchemaId: "slopcamera.operation.analysis.music.input/v1",
    kind: "analysis.music",
    lifecycle: "local-artifact",
    outputSchemaId: "slopcamera.operation.analysis.music.output/v1",
    policy: {
      cache: "none",
      cancellable: true,
      effect: "local-derived-write",
  ...
  {
    inputSchemaId: "slopcamera.operation.analysis.project-inactivity.input/v1",
    kind: "analysis.project-inactivity",
    lifecycle: "local-artifact",
    outputSchemaId: "slopcamera.operation.analysis.project-inactivity.output/v1",
    policy: {
      cache: "none",
      cancellable: true,
      effect: "local-derived-write",
...
const selectCuts = defineCompute({
  ...
  inputSchemaId: "slopcamera.direct.select-cuts.input/v1",
  key: "direct.select-cuts",
  maxDurationMs: 2_000,
  maxInputBytes: 256 * 1_024,
  maxOutputBytes: 32 * 1_024,
  outputSchema: SelectCutsOutputSchema,
  outputSchemaId: "slopcamera.direct.select-cuts.output/v1",
  run: () => ({
...
const workflowFixture = defineWorkflow({
  ...
  inputSchemaId: "slopcamera.direct.code-mode.input/v1",
  version: 1,
});
...
export function createWorkflowEvidence(): WorkflowEvidence {
  ...
  return parseWorkflowEvidence({
    ...
    recovery: {
      command: "slopcamera runs resume run_direct_workflow --replay-ambiguous-code curate",
      failedNode: "curate",
    ...
    runs: [
      {
        ...
        summary: {
          ...
          version: "slopcamera-run-store-v2",
        },
      },
      {
