...
const scene=parseSpatialScene({kind:"atet.spatial-scene",schemaVersion:1,sceneId:"scene_world_sample",coordinates:"right-handed-y-up-meters",durationUs:3_000_000,entities:[imported.entity],assets:imported.assets,cameras:[camera],generators:[],overrides:[],animations:[
...
if(argv.includes("--preflight")){const report={kind:"atet.saved-world-preflight",source:{...originalIntegrity,url:publicWorld.url},imported,hybridScene,hybridPath,nativeInvocations:0};await writeFile(join(root,"preflight.json"),JSON.stringify(report,null,2)+"\n");console.log(JSON.stringify({passed:true,preflight:join(root,"preflight.json")}));process.exit(0);}
...
async function render(name:string,path:string,request:SpatialRenderRequest,assets?:{assetId:string;artifact:{path:string;sha256:string;bytes:number}}[],signal=new AbortController().signal){
 ...
 const identity={nodeKey:name,nodePlanSha256:hash(name),runId:"world-qualification",kind:"scene.render" as const,version:1,inputSchemaId:"atet.operation.scene.render.input/v1",outputSchemaId:"atet.operation.scene.render.output/v1"};
...
const hybridScene=parseSpatialScene({kind:"atet.spatial-scene",schemaVersion:1,sceneId:"scene_hybrid_control",coordinates:"right-handed-y-up-meters",durationUs:2_000_000,
