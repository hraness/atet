...
async function fixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), "slopcamera-browser-runtime-"));
  roots.push(root);
...
describe("complete HTML-overlay browser runtime binding", () => {
  ...
  test("rejects a standalone or wrapper executable as an incomplete browser runtime", async () => {
    const root = await mkdtemp(join(tmpdir(), "slopcamera-browser-wrapper-"));
    roots.push(root);
  ...
  test("rejects an app-shaped shell wrapper without trusted native provenance", async () => {
    const root = await mkdtemp(join(tmpdir(), "slopcamera-browser-shaped-wrapper-"));
    roots.push(root);
