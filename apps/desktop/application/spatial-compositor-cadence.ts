...
export function createSpatialCompositorCadence(input: { readonly projection: unknown; readonly projectionSha256: string; readonly plan: unknown }) {
  ...
  if (canonicalJsonSha256({ domain: "atet.scene-render-projection/v1", projection }) !== input.projectionSha256) throw new Error("Spatial render projection digest mismatch.");
  ...
  const cadence = {
    kind: "atet.spatial-compositor-cadence" as const, schemaVersion: 1 as const,
