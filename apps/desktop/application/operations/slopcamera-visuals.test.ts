...
async function fixture(): Promise<Readonly<{
  ...
  const root = await mkdtemp(join(tmpdir(), "slopcamera-visual-operation-"));
  roots.push(root);
...
describe("unified Slopcamera visual operations", () => {
  test("reuses one capacity-one scheduler lease for default diagram operations", async () => {
    ...
    const processCoordinator = createProcessLocalHostResourceCoordinator({
      profile: {
        ...
        id: `slopcamera.visual-operation-test/${sha256(testFixture.root).slice(0, 16)}`,
      },
    ...
    registry.register(createSlopcameraDiagramCheckOperationDefinition({
      hostResourceCoordinator: coordinator,
    }));
    registry.register(createSlopcameraDiagramRenderOperationDefinition({
      hostResourceCoordinator: coordinator,
    }));
    const execute = async (
      kind: "slopcamera.diagram.check" | "slopcamera.diagram.render",
    ...
    expect((await execute("slopcamera.diagram.check")).summary.kind)
      .toBe("slopcamera.diagram.check");
    expect((await execute("slopcamera.diagram.render")).summary.kind)
      .toBe("slopcamera.diagram.render");
  ...
  test("accepts progressive paths and exact byte claims for every visual file input", () => {
    ...
    expect(SlopcameraDiagramCheckInputSchema.parse({ path: exact }).path)
      .toEqual(exact);
    expect(SlopcameraDiagramRenderInputSchema.parse({ path: "scraps/diagram.json" }))
      .toEqual({ path: "scraps/diagram.json" });
    expect(SlopcameraImageVectorizeInputSchema.parse({ inputPath: exact }).inputPath)
      .toEqual(exact);
    expect(SlopcameraDiagramRenderInputSchema.safeParse({
  ...
  test("rejects an integrity-bound diagram when its bytes change before execution", async () => {
    const testFixture = await fixture();
    const exactInput = await bindSlopcameraVisualOperationInput(
      testFixture.context.application,
  ...
  test("checks an immutable diagram snapshot across a concurrent source edit", async () => {
    ...
    const output = SlopcameraDiagramCheckOutputSchema.parse((await registry.execute(
      testFixture.context,
      {
        input: { path: testFixture.diagramPath },
        kind: "slopcamera.diagram.check",
  ...
  test("renders an immutable diagram snapshot across a concurrent source edit", async () => {
    ...
    const output = SlopcameraDiagramRenderOutputSchema.parse((await registry.execute(
      testFixture.context,
      {
        input: { path: testFixture.diagramPath },
  ...
  test("checks a portable diagram through its integrity-bound application input", async () => {
    ...
    expect(result.summary.kind).toBe("slopcamera.diagram.check");
  ...
  test("publishes every diagram derivative by content hash and exposes composable media references", async () => {
    ...
    expect(Object.values(output.artifacts).every(artifact => (
      artifact.path.startsWith("artifacts/slopcamera/generated/media-operations/outputs/")
    ))).toBe(true);
    expect(output.receipt.path).toStartWith(
      "artifacts/slopcamera/generated/media-operations/receipts/",
    );
  ...
  test("recovers diagram renders only while the exact receipt and every output remain intact", async () => {
    ...
    const exactInput = BoundSlopcameraDiagramRenderInputSchema.parse(
    ...
    const request = {
      ...
      identity: {
        ...
        kind: "slopcamera.diagram.render" as const,
  ...
  test("publishes a locally vectorized SVG and its exact quality receipt", async () => {
    ...
    registry.register(createSlopcameraImageVectorizeOperationDefinition({
    ...
    const result = await registry.execute({
      ...
      application: {
        ...
        hostResourceLease: {
          ...
          profile: {
            capacities: [],
            id: "slopcamera-visuals-test",
          },
          ticket: "slopcamera-visuals-test-ticket",
        },
      },
    }, {
    ...
    expect(output.artifact.path).toMatch(
      /^artifacts\/slopcamera\/generated\/media-operations\/outputs\/[a-f0-9]{64}\.svg$/u,
    );
  ...
  test("recovers adaptive alpha selection only while exact input, receipt, and SVG agree", async () => {
    ...
    const exactInput = BoundSlopcameraImageVectorizeInputSchema.parse(
      await bindSlopcameraVisualOperationInput(
        context.application,
    ...
    const output = SlopcameraImageVectorizeOutputSchema.parse((await registry.execute(
      context,
      {
        input: exactInput,
        kind: "slopcamera.image.vectorize",
    ...
    const request = {
      ...
      identity: {
        ...
        kind: "slopcamera.image.vectorize" as const,
        nodeKey: context.workflow.nodeKey,
        nodePlanSha256: context.workflow.nodePlanSha256,
    ...
    for (const [index, vectorizer] of [
      ...
      const receipt = await publishContentAddressedReceipt({
        ...
        receipt: SlopcameraImageVectorizeReceiptSchema.parse({
      ...
      const mismatchedOutput = SlopcameraImageVectorizeOutputSchema.parse({
        ...output,
        receipt,
        vectorizer,
      });
