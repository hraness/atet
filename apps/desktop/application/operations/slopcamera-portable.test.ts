...
import { PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS } from "@hraness/slopcamera/code/advanced";
import {
  lstat,
  mkdtemp,
  readdir,
...
import { createSlopcameraPortableOperationDefinitions } from "./slopcamera-portable";
...
function registryWith(
  dependencies: Parameters<
    typeof createSlopcameraPortableOperationDefinitions
  >[0],
): OperationRegistry {
...
describe("portable Slopcamera application operations", () => {
  test("parses paths and delegates exactly once beneath an inherited host lease", async () => {
    ...
    const lease: ApplicationHostResourceLease = {
      ...
      profile: { capacities: [], id: "slopcamera.test-portable/v1" },
      ticket: "1",
    };
  ...
  test("serializes portable writers across the shared machine-state mutation lease", async () => {
    const root = await mkdtemp(join(tmpdir(), "slopcamera-portable-output-lease-"));
    ...
    try {
      ...
      expect((await lstat(join(
        ...
        "portable-output-publication-leases",
        leaseRoot,
        ".slopcamera-mutation.lock",
      ))).isFile()).toBe(true);

      const childMarker = join(root, "child-executed");
      const childSource = `
        import { join } from "node:path";
        import { createSlopcameraPortableOperationDefinitions } from ${JSON.stringify(
          new URL("./slopcamera-portable.ts", import.meta.url).href,
        )};
