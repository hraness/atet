...
const IMAGE_SOURCE = {
  ...
  mediaType: "image/png",
  path: "artifacts/slopcamera/private/gateway/input.png",
  sha256: HASH_A,
} as const;
...
const AUDIO_SOURCE = {
  ...
  mediaType: "audio/wav",
  path: "artifacts/slopcamera/private/gateway/input.wav",
  sha256: HASH_B,
} as const;
...
function gatewayResult(
  ...
  return GatewayOperationResultSchema.parse({
    ...
    outputs: [{
      ...
      path: `artifacts/slopcamera/generated/gateway/${operation}/${HASH_A}`,
      sha256: HASH_A,
    }],
    receipt: {
      bytes: 512,
      path: `artifacts/slopcamera/generated/gateway/${operation}/${HASH_B}.json`,
      sha256: HASH_B,
    },
    ...
    ...(options.workflow === false
      ...
      : {
          workflow: {
            ...
            runId: "run_gateway01",
            workspaceDirectory:
              `/private/repository/artifacts/slopcamera/private/runs/${nodeKey}`,
          },
        }),
  };
}
  ...
  test("dispatches every modality once through the host port with an exact stable request identity", async () => {
    ...
    for (const request of REQUESTS) {
      ...
      expect(result.receiptReference).toBe(
        `artifacts/slopcamera/generated/gateway/${request.operation}/${HASH_B}.json`,
      );
  ...
  test("fails closed on paid-media shapes the host service cannot execute exactly", () => {
    ...
    expect(GatewayOperationResultSchema.safeParse({
      ...
      outputs: [{
        bytes: 4_096,
        mediaType: "video/mp4",
        path: `artifacts/slopcamera/generated/gateway/image/${HASH_A}`,
        sha256: HASH_A,
      }],
      receipt: {
        bytes: 512,
        path: `artifacts/slopcamera/generated/gateway/image/${HASH_B}.json`,
        sha256: HASH_B,
      },
