...
} from "@hraness/slopcamera/operations";
...
import { MAXIMUM_DIAGRAM_SOURCE_BYTES } from "./slopcamera-visuals";
import { bindRepositoryMedia } from "./media/shared";

type PortableOperationContract =
  typeof PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS[PortableSlopcameraOperationKind];

const portableOperationContracts = Object.freeze(
  PORTABLE_SLOPCAMERA_OPERATION_KINDS.map(
    kind => PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS[kind],
  ),
);
...
export interface SlopcameraPortableOperationDependencies {
...
function inputRecord(value: unknown): Readonly<Record<string, unknown>> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new ApplicationError(
      "invalid-data",
      "Portable Slopcamera operation input must be an object.",
    );
  }
...
function portableOutputTarget(
  ...
  if (kind === "slopcamera.diagram.render") {
...
export function slopcameraPortableOutputPublicationParent(
  kind: PortableSlopcameraOperationKind,
  inputValue: unknown,
): string | undefined {
  ...
  const parent = kind === "slopcamera.diagram.render"
    ? target
    : dirname(target);
...
async function portableOutputLeaseDirectory(
  ...
  const key = sha256Hex(
    `slopcamera.portable-output-publication-lease/v1\0${publicationParent}`,
  );
...
async function withPortableOutputPublicationLease<Value>(
  ...
  return await withPortableOutputQueue(directory, async () =>
    await withMutationLock(directory, {
      command: "workflow:slopcamera-portable",
...
async function assertPortableSnapshotIntegrity(
  ...
  const source = kind === "slopcamera.diagram.check"
    || kind === "slopcamera.diagram.render"
    ? input.path
    : kind === "slopcamera.image.vectorize"
      ? input.inputPath
  ...
  const sourceKind = kind === "slopcamera.image.vectorize"
    ? "vector"
    : "diagram";
...
async function openPinnedPortableSnapshot(
  ...
  try {
    ...
    const processPath = kind === "slopcamera.image.vectorize"
      ? "/dev/fd/3"
      : descriptorPath;
    const pinnedInput = kind === "slopcamera.image.vectorize"
 ...
 */
export async function bindSlopcameraPortableOperationInputV2(
  application: ApplicationContext,
  kind: PortableSlopcameraOperationKind,
  inputValue: unknown,
...
function assertPortableProjectionIntegrity(): void {
  const contractKinds = Object.keys(PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS)
    .sort((left, right) => left.localeCompare(right));
  const portableKinds = [...PORTABLE_SLOPCAMERA_OPERATION_KINDS]
    .sort((left, right) => left.localeCompare(right));
  ...
  if (
    PORTABLE_SLOPCAMERA_OPERATION_KINDS.length !== PORTABLE_OPERATION_COUNT
    || !isDeepStrictEqual(contractKinds, portableKinds)
    || !Object.isFrozen(PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS)
    ...
    throw new ApplicationError(
      "incompatible",
      "The shared portable Slopcamera contracts drifted from their public workflow projection.",
    );
  }
}
...
async function executePortableOperation(
  ...
  input: unknown,
  dependencies: SlopcameraPortableOperationDependencies,
): Promise<unknown> {
  ...
  const execute = async (): Promise<unknown> => {
    ...
    try {
      ...
      const parsedInput = (dependencies.parseInput ?? parseSlopcameraOperationInput)(
        contract.kind,
        executionInput,
      );
      ...
      const executionDependencies: SlopcameraOperationDependencies = {
      ...
      const output = context.application.hostResourceLease === undefined
        ? await (dependencies.execute ?? executeSlopcameraOperation)(
            contract.kind,
            parsedInput,
            executionDependencies,
          )
        : await (dependencies.executeWithLease ?? executeSlopcameraOperationWithLease)(
            contract.kind,
            parsedInput,
...
export function createSlopcameraPortableOperationDefinitions(
  dependencies: SlopcameraPortableOperationDependencies = {},
...
export const slopcameraPortableOperationDefinitions =
  createSlopcameraPortableOperationDefinitions();
