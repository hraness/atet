...
async function fixtureRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "slopcamera-media-operation-"));
  roots.push(root);
  return root;
}
...
describe("media application operations", () => {
  ...
  test("audio and color effects publish verified content addresses and receipts", async () => {
    ...
    const sourceDirectory = join(root, "artifacts", "slopcamera", "recordings");
  ...
  test("ingest returns an authority-free immutable candidate asset", async () => {
    ...
    const inputDirectory = join(root, "artifacts", "slopcamera", "recordings");
    ...
    const importedRelative = `artifacts/slopcamera/projects/${fixture.project.projectId}/imports/${importedSha256}.media`;
    ...
    )).toMatchObject({ kind: "slopcamera.local-media-ingest-receipt" });
