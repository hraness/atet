...
export function studioOperationFixture() {
  ...
  const bundle = { kind: "atet.studio-source-bundle", schemaVersion: 1, engine: "cadquery", entrypoint: { kind: "python", path: "part.py" }, files: [{ path: "part.py", bytes: 32, sha256: sha }] };
  const job = {
    kind: "atet.studio-job", schemaVersion: 1, jobId: "studio_fixture", bundleSha256: studioSourceBundleSha256(bundle), stage: "build", parameters: {},
  ...
  const runtime = {
    kind: "atet.studio-runtime", schemaVersion: 1, engine: "cadquery", tool: { name: "Python", version: "fixture", executableSha256: sha }, driverSha256: sha,
  ...
  const input = BoundStudioRunInputSchema.parse({ bundle: { path: `artifacts/atet/private/studio/bundles/${plan.bundleSha256}/bundle.json`, bytes: 200, sha256: sha }, job: plan.job, plan });
  const output = StudioRunOutputSchema.parse({
    receipt: { path: "artifacts/atet/private/studio/jobs/studio_fixture/receipt.json", bytes: 600, sha256: sha },
    document: { kind: "atet.studio-receipt", schemaVersion: 1, jobId: job.jobId, attemptId: "attempt_fixture", planSha256: plan.planSha256, bundleSha256: plan.bundleSha256,
    ...
    outputs: [{ artifact: { path: "artifacts/atet/private/studio/jobs/studio_fixture/outputs/part.step", bytes: 100, sha256: sha }, outputId: "part", role: "model", format: "step" }],
  });
  return { input, output };
}
