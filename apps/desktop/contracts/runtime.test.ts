...
test("desktop runtime writes the Slopcamera protocol while reading the Studio protocol", () => {
  expect(SLOPCAMERA_DESKTOP_PROTOCOL_VERSION).toBe(3);
  const request = {
  ...
  expect(DesktopRequestSchema.parse({
    ...request,
    protocolVersion: 3,
  }).protocol).toBe("slopcamera.desktop");
  expect(DesktopRequestSchema.parse({
    ...request,
