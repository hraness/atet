...
export function resolveZigExecutable(
  ...
  throw new Error(
    "Zig 0.16.0 is required for Slopcamera macOS development. Install it with `brew install zig`, set NATIVE_SDK_ZIG to an absolute Zig executable, or run `bun x native build apps/desktop --yes` once to install Native SDK's managed toolchain.",
  );
}
