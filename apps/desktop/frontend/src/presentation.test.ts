...
describe("recorder presentation", () => {
  test("exposes only legal controls for each authoritative state", () => {
    const cases: readonly [CaptureRuntimeState, readonly RecorderAction[]][] = [
      ...
      [CaptureRuntimeStateSchema.parse({ commandId: "command_start001", recordingPath: "artifacts/slopcamera/recordings/rec_pending01", state: "starting" }), []],
      [CaptureRuntimeStateSchema.parse({ recordingId: "rec_active001", recordingPath: "artifacts/slopcamera/recordings/rec_active001", sourceTimeUs: 5, state: "recording" }), ["pause", "stop"]],
      [CaptureRuntimeStateSchema.parse({ recordingId: "rec_active001", recordingPath: "artifacts/slopcamera/recordings/rec_active001", sourceTimeUs: 5, state: "paused" }), ["resume", "stop"]],
      [CaptureRuntimeStateSchema.parse({ commandId: "command_stop0001", recordingId: "rec_active001", recordingPath: "artifacts/slopcamera/recordings/rec_active001", sourceTimeUs: 5, state: "stopping" }), []],
      [CaptureRuntimeStateSchema.parse({ code: "capture-session-failed", message: "raw", recordingId: "rec_active001", recordingPath: "artifacts/slopcamera/recordings/rec_active001", sourceTimeUs: 5, state: "failed" }), ["start"]],
  ...
  test("explains an automatic pause without leaking native interruption evidence and keeps explicit resume legal", () => {
    ...
    expect(presentationForSnapshot(snapshot)).toEqual({
      ...
      path: "artifacts/slopcamera/recordings/rec_active001",
      tone: "pending",
    });
  ...
  test("uses owned source-specific failure copy and never offers resume", () => {
    ...
    const snapshot = CaptureRuntimeSnapshotSchema.parse({
      ...
      state: {
        ...
        recordingPath: "artifacts/slopcamera/recordings/rec_active001",
        sourceTimeUs: 2_000_000,
        state: "failed",
      },
    });
  ...
  test("maps untrusted command and failure diagnostics to owned UI copy", () => {
    ...
    expect(presentationForSnapshot(snapshot)).toMatchObject({
      allowedActions: [],
      label: "Slopcamera needs a configured Slopcamera checkout",
    });
    ...
    expect(presentationForSnapshot(incompleteRecovery)).toMatchObject({
      allowedActions: [],
      label: "Recording recovery incomplete — restart Slopcamera to record again",
    });
