...
export function Button({
  ...
  return (
    <span className={["slopcamera-button", className].filter(Boolean).join(" ")}>
      <button
        {...props}
        className="slopcamera-button__control"
        data-size={size}
...
export function SearchField({ className, label, onChange, size, surface, ...props }: SearchFieldProps) {
  return (
    <label className={["slopcamera-search", className].filter(Boolean).join(" ")} data-size={size} data-surface={surface}>
      <span className="slopcamera-search__label">{label}</span>
