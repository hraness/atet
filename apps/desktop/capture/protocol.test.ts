...
describe("capture helper request protocol", () => {
  test("parses configure from unknown and applies bounded defaults", () => {
    const request = parseCaptureRequestLine(JSON.stringify({
      ...
      requestId: "request-1",
      sessionDirectory: "/tmp/slopcamera-project/artifacts/slopcamera/recordings/rec_00000001",
    }));
  ...
  test("retains exact source selections and rejects duplicate or oversized display sets", () => {
    const selected = parseCaptureRequestLine(JSON.stringify({
      ...
      sessionDirectory: "/tmp/slopcamera-project/artifacts/slopcamera/recordings/rec_selected",
    }));
    ...
    expect(() => parseCaptureRequestLine(JSON.stringify({
      ...
      sessionDirectory: "/tmp/slopcamera-project/artifacts/slopcamera/recordings/rec_duplicate",
    }))).toThrow(/unique/u);
    expect(() => parseCaptureRequestLine(JSON.stringify({
      command: "configure",
      options: {
      ...
      sessionDirectory: "/tmp/slopcamera-project/artifacts/slopcamera/recordings/rec_too_many",
    }))).toThrow();
  });
  ...
  test("strictly binds optional typed text to complete focus identities", () => {
    ...
    for (const key of [
      ...
      expect(() => parseCaptureRequestLine(JSON.stringify({
        ...
        sessionDirectory:
          "/tmp/slopcamera-project/artifacts/slopcamera/recordings/rec_00000001",
      }))).toThrow();
    }
    expect(() => parseCaptureRequestLine(JSON.stringify({
      command: "configure",
