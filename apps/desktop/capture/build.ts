...
const captureHelperIdentifier = "com.hraness.slopcamera.capture";
export const captureHelperExecutablePath = join(outputDirectory, "slopcamera-capture");
...
async function sourceHash(toolchain: Toolchain, sources: readonly string[]): Promise<string> {
  ...
  hash.update("slopcamera-capture-build-v2\0");
...
export async function runCaptureSegmentCloseGateHarness(
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "slopcamera-capture-close-gate-"));
...
export async function runCaptureControllerFinalizationHarness(
  ...
  const toolchain = await discoverCaptureToolchain();
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "slopcamera-capture-finalization-"));
  const harnessPath = join(temporaryDirectory, "Harness.swift");
...
async function installStableExecutable(cachedExecutable: string): Promise<void> {
  const temporary = join(outputDirectory, `.slopcamera-capture.${process.pid}.${crypto.randomUUID()}.tmp`);
  try {
...
export async function buildCaptureHelper(): Promise<BuildResult> {
  ...
  const cachedExecutable = join(hashedDirectory, "slopcamera-capture");
  ...
  if (!cached) {
    const temporary = join(hashedDirectory, `.slopcamera-capture.${process.pid}.${crypto.randomUUID()}.tmp`);
    try {
      await runCompiler(
        [
          toolchain.swiftCompiler,
