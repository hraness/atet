...
describe("owned interaction fixture protocol", () => {
  ...
  test("binds ready, exercise, receipt, and shutdown to one exact identity", async () => {
    ...
    const fixture = await startInteractionFixture({
      executable: "/tmp/slopcamera-capture",
      fixtureId,
      transportFactory: factory,
    });
    expect(factory.spawned).toEqual(["/tmp/slopcamera-capture", fixtureId]);
  ...
  test("rejects nonce drift at ready and every receipt focus-identity drift", async () => {
    const drift = {
      fieldId: "slopcamera-fixture-public-fedcba98-7654-4abc-9def-fedcba987654",
      processId: 99,
      windowId: "99",
      windowTitle:
        "Slopcamera Interaction Fixture · fedcba98-7654-4abc-9def-fedcba987654",
    } as const;
