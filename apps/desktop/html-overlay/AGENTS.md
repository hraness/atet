...
- Emit `slopcamera.*` identities for new authoring state. Accept a named Slopcamera value only through an explicit compatibility parser or normalization path that keeps persisted v1 work readable.
- Resolve new authoring only through the active exact lock registry. Historical locks are append-only receipt evidence, not an executable selection surface. Never accept ranges, redirects, arbitrary subpath imports, mutable URLs, or caller-supplied lock metadata. The Spark spatial profile may include the exact host-owned `three/addons/postprocessing/Pass.js` dependency lock; this does not authorize other Three subpaths.
...
- Keep author code on an absolute Slopcamera-owned clock. The host seeks browser and tracked animations before invoking frame callbacks; visible output must not depend on wall time.
