...
- Emit `transmute.*` identities for new authoring state. Accept a named Transmute value only through an explicit compatibility parser or normalization path that keeps persisted v1 work readable.
...
- Keep author code on an absolute Transmute-owned clock. The host seeks browser and tracked animations before invoking frame callbacks; visible output must not depend on wall time.
