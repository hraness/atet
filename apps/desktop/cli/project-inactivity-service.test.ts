...
async function projectFixture(repositoryRoot: string): Promise<OpenProject> {
  const recordingDirectory = join(repositoryRoot, "artifacts", "slopcamera", "recordings", "rec_example001");
  ...
  const recording = await openRecording(
    join(repositoryRoot, "artifacts", "slopcamera", "recordings"),
...
describe("project inactivity analysis", () => {
  test("intersects every enabled screen and audio stream and protects mapped recording interactions", async () => {
    const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-inactivity-"));
    try {
      ...
      expect(result.analysis.kind).toBe("slopcamera.project-inactivity-analysis");
  ...
  test("omits audio probes and evidence when silence protection is disabled", async () => {
    const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-inactivity-no-audio-"));
    try {
      const project = await projectFixture(repositoryRoot);
      const runner = new InactivityRunner();
      const result = await analyzeAndPersistProjectInactivity({
  ...
  test("rejects project media symlinks that resolve outside the repository", async () => {
    const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-inactivity-path-"));
    const outsideRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-inactivity-outside-"));
    try {
      const project = await projectFixture(repositoryRoot);
