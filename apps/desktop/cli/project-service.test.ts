...
async function recordingFixture(repositoryRoot: string): Promise<OpenRecording> {
  const recordingRoot = join(repositoryRoot, "artifacts", "slopcamera", "recordings", "rec_example001");
  ...
  return await openRecording(join(repositoryRoot, "artifacts", "slopcamera", "recordings"), "rec_example001");
}
  ...
  return ProjectAssetV1Schema.parse({
    ...
    streams: [{
      ...
      segments: [{
        ...
        path: "artifacts/slopcamera/projects/project_multicam1/imports/second-angle.mov",
        sha256: HASH,
        streamIndex: 0,
      }],
...
describe("project creation from recording bundles", () => {
  test("persists every co-clocked screen, camera, and audio stream with an identity placement", async () => {
    const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-service-"));
    try {
      ...
      expect(created.project.kind).toBe("slopcamera.video-project");
      ...
      expect(asset.streams.flatMap(stream => stream.segments.map(segment => segment.path)))
        .toEqual([
          "artifacts/slopcamera/recordings/rec_example001/media/segment-1.mp4",
          "artifacts/slopcamera/recordings/rec_example001/media/segment-left.mp4",
          "artifacts/slopcamera/recordings/rec_example001/media/segment-1.mp4",
          "artifacts/slopcamera/recordings/rec_example001/media/segment-1.mp4",
          "artifacts/slopcamera/recordings/rec_example001/media/camera.mov",
        ]);
  ...
  test("rejects active recordings and recording paths outside the declared repository", async () => {
    const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-boundary-"));
    try {
      const recording = await recordingFixture(repositoryRoot);
      const projectRoot = join(repositoryRoot, "artifacts", "slopcamera", "projects");
...
test("adds imported media as an explicitly unverified placement and rebases the full-tail plan", async () => {
  const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-add-"));
  try {
    const projectRoot = join(repositoryRoot, "artifacts", "slopcamera", "projects");
    const created = await createProjectFromRecording({
      id: "project_multicam1",
...
test("keeps every video stream disabled when an imported asset has an audio-only role", async () => {
  const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-audio-role-"));
  try {
    const projectRoot = join(repositoryRoot, "artifacts", "slopcamera", "projects");
    const created = await createProjectFromRecording({
...
test("reuses content-identical imported assets across invocation timestamps", async () => {
  const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-project-reimport-"));
  try {
    const projectRoot = join(repositoryRoot, "artifacts", "slopcamera", "projects");
    const created = await createProjectFromRecording({
      id: "project_multicam1",
