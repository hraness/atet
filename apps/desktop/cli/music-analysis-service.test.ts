...
async function createFixture(): Promise<{
  ...
  const repositoryRoot = await mkdtemp(join(tmpdir(), "slopcamera-music-service-"));
  const mediaDirectory = join(repositoryRoot, "imports");
  const projectDirectory = join(repositoryRoot, "artifacts", "slopcamera", "projects", "project_musicfixture");
...
describe("bounded PCM reading", () => {
  test("streams finite little-endian floats without reading beyond its byte bound", async () => {
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-music-pcm-"));
  ...
  test("rejects partial and non-finite float samples", async () => {
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-music-invalid-pcm-"));
    const path = join(temporary, "audio.f32le");
    try {
...
test("persists a validated immutable music sidecar and compact project reference", async () => {
  ...
  try {
    ...
    expect(result.analysis.kind).toBe("slopcamera.music-analysis");
