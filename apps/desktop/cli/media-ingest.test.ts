...
test("imports external media once by content hash and models every probed stream", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-ingest-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const projectDirectory = join(requestedRepositoryRoot, "artifacts", "slopcamera", "projects", "project_ingest01");
...
test("syncs staged media before publication and directory entries after publication and cleanup", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-durability-order-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
...
test("does not publish media when syncing the staged inode fails", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-durability-failure-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const requestedProjectDirectory = join(
      requestedRepositoryRoot,
...
test.skipIf(FFMPEG === null || FFPROBE === null)(
  ...
  async () => {
    ...
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-webm-ingest-"));
    try {
      const repositoryRoot = join(temporary, "repository");
      const projectDirectory = join(repositoryRoot, "artifacts", "slopcamera", "projects", "project_webmprobe");
...
test("rejects symlink inputs and symlinked import directories", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-symlink-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const projectDirectory = join(requestedRepositoryRoot, "artifacts", "slopcamera", "projects", "project_symlink01");
    const source = join(temporary, "source.mov");
...
test("rejects a project directory outside the declared repository before staging or probing", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-project-boundary-"));
  try {
    const repositoryRoot = join(temporary, "repository");
...
test("detects corruption at an existing content-addressed destination", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-collision-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const projectDirectory = join(requestedRepositoryRoot, "artifacts", "slopcamera", "projects", "project_collision1");
    const source = join(temporary, "audio.wav");
    await mkdir(projectDirectory, { recursive: true });
    const repositoryRoot = await realpath(requestedRepositoryRoot);
...
test("rejects a symlink prepositioned at a content-addressed destination", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-destination-link-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const projectDirectory = join(requestedRepositoryRoot, "artifacts", "slopcamera", "projects", "project_destlink1");
...
test("removes staged imports after a failed probe without touching an existing content-addressed file", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "slopcamera-media-probe-failure-"));
  try {
    const requestedRepositoryRoot = join(temporary, "repository");
    const projectDirectory = join(requestedRepositoryRoot, "artifacts", "slopcamera", "projects", "project_probe_fail1");
    const imports = join(projectDirectory, "imports");
    const source = join(temporary, "audio.wav");
    await mkdir(imports, { recursive: true });
    const repositoryRoot = await realpath(requestedRepositoryRoot);
