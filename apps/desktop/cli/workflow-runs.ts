...
function schedulerOwner(): string {
  return `slopcamera-cli:${String(process.pid)}:${randomUUID()}`;
}
...
function localCliAuthorization(): SchedulerAuthorization {
  return {
    ...
    grantedBy: "slopcamera-local-cli",
  };
}
...
export function humanRunSummary(
  ...
  if (result.pause !== undefined) {
    const option = result.pause.phase === "effect"
      ...
      : "--preparation-plan";
    lines.push(
      `next slopcamera runs approve ${summary.runId} ${result.pause.nodeKey} ${option} ${result.pause.planSha256}`,
      `then slopcamera runs resume ${summary.runId}`,
    );
  } else if (
    summary.status !== "completed"
    && summary.status !== "cancelled"
  ) {
    lines.push(`next slopcamera runs show ${summary.runId} --nodes failed`);
  }
  return lines.join("\n");
}
