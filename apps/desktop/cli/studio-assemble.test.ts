...
describe("native studio ordinary project assembly", () => {
  test("uses observed rounded container duration while cutting exact fractional video time, and reuses replay", async () => {
    ...
    expect(first.kind).toBe("slopcamera.studio-assembly");
...
test.skipIf(process.env.SLOPCAMERA_STUDIO_ENCODE_NATIVE !== "1")("native NTSC sequence assembles using actual container coverage and exact video cut", async () => {
