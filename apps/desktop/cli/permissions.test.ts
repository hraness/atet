...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-permissions-test-"));
    const recordingRoot = join(temporary, "artifacts", "slopcamera", "recordings", "rec_permissions01");
    try {
