...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-control-test-"));
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const temporary = await mkdtemp(
      join(tmpdir(), "slopcamera-control-response-fallback-test-"),
    );
    const socketPath = join(temporary, "control.sock");
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const temporary = await mkdtemp(join(tmpdir(), "slopcamera-control-scope-test-"));
    const socketPath = join(temporary, "control.sock");
    const focusIdentity = {
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const artifactRoot = await mkdtemp(join(tmpdir(), "slopcamera-control-recovery-test-"));
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      const artifactRoot = await mkdtemp(
        join(tmpdir(), "slopcamera-control-bounded-recovery-test-"),
      );
      const socketPath = join(artifactRoot, ".recording-control.sock");
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const artifactRoot = await mkdtemp(join(tmpdir(), "slopcamera-stale-control-test-"));
