...
async function recordingFixture(): Promise<RecordingFixture> {
  const root = await mkdtemp(join(tmpdir(), "slopcamera-mutation-lock-"));
...
describe("exclusive bundle mutation lock", () => {
  test("first mutation of a predecessor recording plan receipts the saved Slopcamera bytes", async () => {
    ...
    try {
      ...
      expect(saved.kind).toBe("slopcamera.edit-plan");
  ...
  test("uses the same conflict boundary for project edits", async () => {
    const root = await mkdtemp(join(tmpdir(), "slopcamera-project-mutation-lock-"));
    const paths: RepositoryPaths = {
      artifactRoot: join(root, "artifacts", "slopcamera", "recordings"),
      desktopRoot: join(root, "projects", "slopcamera", "apps", "desktop"),
      privateRoot: join(root, "artifacts", "slopcamera", "private"),
  ...
  test("publishes only a complete owner and cleans proven-abandoned acquisition temps", async () => {
    const directory = await mkdtemp(join(tmpdir(), "slopcamera-published-lock-"));
  ...
  test("recovers old zero-byte and partial legacy acquisition artifacts only when proven unheld", async () => {
    for (const [name, contents] of [["empty", ""], ["partial", "{\"acquiredAt\":"]] as const) {
      const directory = await mkdtemp(join(tmpdir(), `slopcamera-${name}-legacy-lock-`));
      try {
  ...
  test("keeps an old partial owner fail-closed when it may still be open", async () => {
    const directory = await mkdtemp(join(tmpdir(), "slopcamera-open-partial-lock-"));
  ...
  test("keeps symlink and non-file lock artifacts fail-closed", async () => {
    const directory = await mkdtemp(join(tmpdir(), "slopcamera-unsafe-lock-"));
    const path = join(directory, MUTATION_LOCK_FILE);
  ...
  test("reclaims only an old same-host owner that is proven dead", async () => {
    const directory = await mkdtemp(join(tmpdir(), "slopcamera-stale-lock-"));
    try {
  ...
  test("never reclaims a live owner merely because the operation is long", async () => {
    const directory = await mkdtemp(join(tmpdir(), "slopcamera-live-lock-"));
