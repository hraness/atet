...
var exports_skill_install = {};
__export(exports_skill_install, {
...
async function main(args) {
  ...
  if (command === "init") {
    ...
    const filePath = resolve5(parsed.positionals[0] ?? "diagram.diagram.json");
    if (await pathExists(filePath))
      throw new Error(`Refusing to overwrite existing file: ${filePath}`);
    await writeFile2(filePath, `${JSON.stringify(starter, null, 2)}
`);
  ...
  if (command === "skill") {
    const [subcommand, ...subcommandArgs] = rest;
    if (subcommand === "path") {
      const { bundledSkillPath: bundledSkillPath2 } = await Promise.resolve().then(() => (init_skill_install(), exports_skill_install));
      console.log(bundledSkillPath2());
      return;
    }
    if (subcommand === "install") {
