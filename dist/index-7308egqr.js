...
async function installSkill(options) {
  ...
  if (await pathExists(legacy)) {
    throw new Error(`Legacy diagram skill found at ${legacy}. Remove or move that directory, then rerun "slopcamera skill install --target ${options.target} --scope ${options.scope}". Slopcamera will not install both skills side by side.`);
  }
