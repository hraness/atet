...
function initializeNativeLocking() {
  ...
  if (process.platform === "linux") {
    ...
    throw new Error("Slopcamera could not load host flock support" + ` (${failures.join("; ")}).`);
  }
  throw new Error(`Slopcamera host-resource coordination is unsupported on ${process.platform}.`);
}
function currentErrno(locking) {
  const pointer = locking.errnoLocation();
  if (pointer === null) {
    throw new Error("Slopcamera could not read the host flock error.");
  }
...
var slopcameraHostResourcePlatforms = Object.freeze([
  "darwin",
  "linux"
]);
function isHostResourcePlatformSupported(platform) {
  return slopcameraHostResourcePlatforms.some((candidate) => candidate === platform);
}
function tryLockHostResourceDescriptor(descriptor) {
  ...
  for (;; ) {
    ...
    if (isBusyErrno(errno))
      return false;
    throw new Error(`Slopcamera host-resource flock failed with errno ${errno}.`);
  }
}
function unlockHostResourceDescriptor(descriptor) {
  ...
  for (;; ) {
    ...
    if (errno === interruptedErrno)
      continue;
    throw new Error(`Slopcamera host-resource unlock failed with errno ${errno}.`);
  }
}
...
var slopcameraHostResourceNames = Object.freeze([
  "cpu",
  "local-io",
  "ffmpeg",
  "video-encode",
...
function defaultSlopcameraHostResourceProfile(hostParallelism = availableParallelism()) {
...
function defaultSlopcameraHostResourceStateRoot(platform = process.platform, environment = process.env, userHome = homedir()) {
  if (!isHostResourcePlatformSupported(platform)) {
    throw new HostResourceError("UNSUPPORTED_PLATFORM", "Slopcamera host-resource coordination requires Darwin or Linux.");
  }
  if (platform === "darwin") {
    return join(userHome, "Library", "Application Support", "Slopcamera", "cli", "host-resources-v1");
  }
...
function assertMatchingProfile(state, profile) {
  if (canonicalProfile(state.profile) === canonicalProfile(profile))
    return;
  throw new HostResourceError("PROFILE_MISMATCH", "The machine-global Slopcamera host-resource profile does not match this process.");
}
...
function resolveCoordinatorOptions(options) {
  ...
  const profile = normalizeHostResourceProfile(options.profile ?? defaultSlopcameraHostResourceProfile());
  const stateRoot = options.stateRoot ?? defaultSlopcameraHostResourceStateRoot();
...
function createProcessLocalHostResourceCoordinator(coordinatorOptions = {}) {
  const profile = normalizeHostResourceProfile(coordinatorOptions.profile ?? defaultSlopcameraHostResourceProfile());
...
export { HOST_RESOURCE_MAX_WAIT_MILLISECONDS, slopcameraHostResourceNames, HostResourceError, normalizeHostResourceProfile, normalizeHostResourceClaims, defaultSlopcameraHostResourceProfile, defaultSlopcameraHostResourceStateRoot, createHostResourceCoordinator, createProcessLocalHostResourceCoordinator, createDefaultHostResourceCoordinator };
