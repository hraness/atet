...
import { mkdir, readFile as readFile2, rename as rename2, rm as rm3, writeFile as writeFile2 } from "fs/promises";
...
function workerEntryPath() {
  ...
  return modulePath.endsWith(".ts") ? join(dirname2(modulePath), "worker.ts") : join(dirname2(modulePath), "vectorize-worker.js");
}
...
async function atomicOperationWrite(path, value) {
  const temporaryPath = join2(dirname3(path), `.${randomUUID2()}.graphics-operation.tmp`);
  try {
