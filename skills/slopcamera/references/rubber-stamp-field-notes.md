...
`slopcamera ai models list --type image --json` and pick a reference-capable model.
...
Do not assume the current directory is a Slopcamera source checkout or use the
agent runner's copied skill path. The [source installation](install.md) supplies `SLOPCAMERA_SOURCE_ROOT`; restore that exact checkout in a new shell. Vercel launches the built Bun entrypoint because it cannot execute the source-install shell function. Run through the linked Vercel project:
...
Remux awkward JPEGs to PNG first if Slopcamera's FFprobe rejects them. If Gateway
decode validation fails but bytes exist under `artifacts/slopcamera/generated/…`,
...
- Prefer `/home/box/media/images/slopcamera/YYYY-MM-DD-<place-slug>-field-note.jpg`
  for Slopcamera library copies.
