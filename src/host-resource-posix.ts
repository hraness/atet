...
function initializeNativeLocking(): NativeLocking {
  ...
  if (process.platform === "linux") {
    ...
    throw new Error(
      "Slopcamera could not load host flock support"
      + ` (${failures.join("; ")}).`,
    )
  }
  throw new Error(
    `Slopcamera host-resource coordination is unsupported on ${process.platform}.`,
  )
}
...
export const slopcameraHostResourcePlatforms = Object.freeze([
  "darwin",
  "linux",
] as const)
...
): platform is (typeof slopcameraHostResourcePlatforms)[number] {
