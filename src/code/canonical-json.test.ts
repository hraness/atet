...
function canonicalError(value: unknown): SlopcameraCodeError {
  try {
    canonicalJson(value)
  } catch (error) {
...
describe("portable canonical JSON", () => {
  ...
  test("keeps the native digest identical to the portable fallback", () => {
    const input = "slopcamera\0portable SHA-256 \ud83c\udfa8"
  ...
  test("rejects sparse arrays instead of collapsing distinct values", () => {
    ...
    expect(() => canonicalJson([undefined])).toThrow(SlopcameraCodeError)
  })
