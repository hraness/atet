...
async function temporaryStateRoot(): Promise<Readonly<{
  ...
  const parent = await mkdtemp(join(tmpdir(), "slopcamera-host-resources-"))
...
describe("Slopcamera host-resource profiles", () => {
  ...
  test("reserves one logical processor on two-vCPU hosts", () => {
    const profile = defaultSlopcameraHostResourceProfile(2)
    expect(profile.id).toBe("slopcamera.host-resources/v1")
  ...
  test("keeps video encodes independently serialized by default", () => {
    ...
    const defaults = defaultSlopcameraHostResourceProfile(12)
...
describeMachineHostResources("machine-global Slopcamera host-resource coordination", () => {
  ...
  test("recovers an abruptly lost process from its released kernel lock", async () => {
    ...
    const program = [
      ...
      "const coordinator = createHostResourceCoordinator({ profile, stateRoot: process.env.SLOPCAMERA_TEST_STATE_ROOT, pollIntervalMilliseconds: 2, waitTimeoutMilliseconds: 2000 })",
    ...
    const child = Bun.spawn([process.execPath, "-e", program], {
      env: {
        ...process.env,
        SLOPCAMERA_TEST_STATE_ROOT: temporary.stateRoot,
      },
      stderr: "pipe",
      stdout: "pipe",
    })
