...
test("a conversion executes a private verified copy after the override path changes", async () => {
  ...
  const work = await mkdtemp(join(tmpdir(), "slopcamera-tool-private-"))
  ...
  try {
    ...
    process.env.SLOPCAMERA_VTRACER_PATH = sourceLink
...
test("a FIFO tool override is rejected without blocking", async () => {
  if (process.platform === "win32") return
  const work = await mkdtemp(join(tmpdir(), "slopcamera-tool-fifo-"))
  const previousOverride = process.env.SLOPCAMERA_VTRACER_PATH
  try {
    const source = join(work, "vtracer")
    expect(Bun.spawnSync(["mkfifo", source]).exitCode).toBe(0)
    process.env.SLOPCAMERA_VTRACER_PATH = source
