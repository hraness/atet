...
import {
  ...
  selectDesktopAsset,
  vectorizeImage,
} from "./graphics-compat/index.js"
import {
  graphicsAuthStatus,
  loginGraphics,
  logoutGraphics,
  requireGraphicsAuthentication,
} from "./graphics-compat/auth.js"
import {
  graphicsImageModels,
  type GraphicsImageModel,
} from "./graphics-compat/discovery.js"
import { generateGraphicsImageFile } from "./graphics-compat/generate.js"
import {
  executeGraphicsOperation,
  graphicsOperationCodes,
  isGraphicsOperationCode,
  searchGraphicsOperations,
} from "./graphics-compat/operations.js"
import {
  installSkill,
  type SkillScope,
  type SkillTarget,
...
export function installLegacyVectorizerEnvironment(): () => void {
  const mappings = [
    ["GRAPHICS_VTRACER_PATH", "TRANSMUTE_VTRACER_PATH"],
    ["GRAPHICS_CACHE_DIR", "TRANSMUTE_CACHE_DIR"],
  ] as const
  const previous = mappings.map(([, canonical]) => [canonical, process.env[canonical]] as const)
  ...
  return () => {
    for (const [canonical, value] of previous) {
      if (value === undefined) delete process.env[canonical]
      else process.env[canonical] = value
    }
  }
}
...
export async function main(
  ...
  if (command === "vectorize") {
    ...
    let result: Awaited<ReturnType<typeof vectorizeImage>>
    try {
      result = await (dependencies.vectorize ?? vectorizeImage)(
