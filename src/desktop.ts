...
export async function installDesktop(options: {
  ...
  const cacheDirectory = join(homedir(), ".cache", "slopcamera", "installers", release.tag_name)
...
export async function openInDesktop(filePath: string): Promise<void> {
  ...
  if (application === null) {
    throw new Error(
      `tldraw Offline is not installed. Run "slopcamera canvas install" or visit ${desktopDownloadPage}`,
    )
  }
  if (hostPlatform() === "darwin") {
