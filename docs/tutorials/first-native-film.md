...
Create a one-second product shot in Blender, inspect its retained frames, and export it through an ordinary ATET video project. You will keep the native source and the encoded clip as separate artifacts.

This lesson requires the **current-source studio commands**, not the v3.2.3 archive. Complete [source installation](../how-to/use-current-source.md), which defines `atet` for the built checkout, and verify `atet help studio` lists `init`, `run`, `encode` and `assemble`. The following setup uses macOS, Blender 5.2.1 LTS, Bun, FFmpeg and FFprobe. Set `ATET_BLENDER_BIN` to your installed Blender executable; the usual application path is shown below. The small preview deliberately uses CPU rendering; no cloud service or GPU is required.
...
```sh
export ATET_BLENDER_BIN="/Applications/Blender.app/Contents/MacOS/Blender"
test -x "$ATET_BLENDER_BIN"
```
...
atet studio bundle product/source.json --json > product-bundle.json
```
...
atet studio plan product/preview.job.json --json
atet studio probe product/preview.job.json --blender-bin "$ATET_BLENDER_BIN" --json
```
...
```sh
atet studio run product/preview.job.json --blender-bin "$ATET_BLENDER_BIN" --allow-trusted-code --json
```
...
```sh
atet studio assemble <studio-id> --output-id beauty --name "First product shot" --json
```
...
atet project render plan <project-id> --width 320 --height 180 --fps 24 --output renders/first-shot.mp4 --json
atet project render run <project-id> --width 320 --height 180 --fps 24 --output renders/first-shot.mp4 --json
```

The output and its receipt are relative to the project directory, not your shell’s working directory. For the default workspace, open `artifacts/atet/projects/<project-id>/renders/first-shot.mp4`; the render result’s FFmpeg invocation also names the full output path. It should contain the complete one-second shot at 320×180. Keep the source bundle, job, native receipt and encoded derivative with the project. To direct another version, edit the source, retain a new bundle and choose a new job ID; the [native authoring guide](../studio.md) covers larger shots, caches and shared assets.
