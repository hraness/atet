...
test("a stable version builds automatically and only an owner dispatch stages the exact npm artifact", async () => {
  ...
  expect(stageJob).toContain("const maximumFiles = 400")
  expect(stageJob).toContain("const maximumPackedBytes = 4_000_000")
  expect(stageJob).toContain("const maximumUnpackedBytes = 10_500_000")
