...
- Preserve the existing Vercel project and its durable `staging` environment, which uses the exact Git branch name `preview` as provider authority. Do not delete, rename, repurpose, or force-push that branch as repository cleanup, and do not advance it merely to match `main`. Keep `main` production and pull-request Preview deployments separate. Follow [the Vercel runbook](docs/vercel.md) and audit the environment, domains, aliases, and remote refs before changing this seam.
