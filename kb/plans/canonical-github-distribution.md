...
4. Review the complete impact and run focused hostile-input and lifecycle tests. Complete the source aggregate under `CONTRIBUTING.md` with fresh complete current-candidate CI or its local fallback, retaining explicit local, native, browser and installation acceptance. The integration owner admits the release; verify live assets and isolated installation before claiming delivered availability.
...
Initial source is main `7a0bd73337db589667121ec66f59c092bc11a5da`. GitHub reports immutable public `v3.2.2`; `v3.2.3` is absent. Relevant consumers retain immutable older GitHub pins, so no coordinated consumer release is required. Complete source CI preserves all `bun run check` phases and now scans generated outputs after each build; routed skips cannot qualify as the final aggregate. The local fallback, five-platform official VTracer matrix and macOS shell tests/package remain. A site change also retains its explicit isolated install and pinned Node/Chromium preview proof.
